...
private:
	...
	uint32_t    m_tilebanks[4]{};
	uint32_t    m_spritebanks[4]{};
...
uint32_t asterix_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	{
		const int prev_tilebank = m_tilebanks[bank];
	...
	{
		const int prev_colorbase = m_layer_colorbase[plane];
...
void asterix_state::control2_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	if (ACCESSING_BITS_0_7)
	{
		...
		m_k056832->set_tile_bank(BIT(data, 5));
...
void asterix_state::protection_w(address_space &space, offs_t offset, uint16_t data, uint16_t mem_mask)
{
	...
	if (offset == 1)
	{
		const uint32_t cmd = (m_prot[0] << 16) | m_prot[1];
		switch (cmd >> 24)
		{
			case 0x64:
			{
