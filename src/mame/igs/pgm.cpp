...
{
	LOGMASKED(LOG_VIDEORAM, "%s: videoram_w %08x: %04x & %04x\n", machine().describe_context(), offset << 1, data, mem_mask);
	if (offset < 0x4000 / 2)
...
{
	LOGMASKED(LOG_Z80, "Z80: ctrl %04x @ %04x %s\n", data, mem_mask, machine().describe_context());

	// these for allow M68K access to Z80 RAM
	if (!m_z80_bus_connect)
	{
		if (data == 0x45d3)
			m_z80_bus_connect = true;
			// TODO: Halt
	}
	else
	{
		if (data == 0x0a0a)
			m_z80_bus_connect = false;
	}
}

void pgm_state::m68k_l1_w(u8 data)
{
	LOGMASKED(LOG_Z80, "SL 1 m68.w %02x %s IRQ\n", data, machine().describe_context());
...
{
	LOGMASKED(LOG_Z80, "SL 3 z80.w %02x %s\n", data, machine().describe_context());
...
{
	map(0x8000, 0x8003).rw(m_ics, FUNC(ics2115_device::read), FUNC(ics2115_device::write));
	map(0x8100, 0x81ff).r(m_soundlatch[2], FUNC(generic_latch_8_device::read)).w(FUNC(pgm_state::z80_l3_w));
...
void pgm_state::pgm_base_mem(address_map &map)
{
	...
	map(0x900000, 0x907fff).mirror(0x0f8000).rw(FUNC(pgm_state::videoram_r), FUNC(pgm_state::videoram_w)).share(m_videoram); // IGS023 VIDEO CHIP
	map(0xa00000, 0xa01fff).mirror(0x0fe000).ram().w(m_palette, FUNC(palette_device::write16)).share("palette");
	map(0xb00000, 0xb0ffff).mirror(0x0f0000).ram().share(m_videoregs); // Video Regs inc. Zoom Table
	map(0xb00000, 0xb00fff).mirror(0x0f0000).readonly().share(m_sprite_buffer); // not writeable?
	map(0xb01000, 0xb0103f).mirror(0x0f0fc0).writeonly().share(m_zoomram); // not readable?
	map(0xb02000, 0xb02001).mirror(0x0f0ffe).rw(FUNC(pgm_state::bg_scrolly_r), FUNC(pgm_state::bg_scrolly_w));
	map(0xb03000, 0xb03001).mirror(0x0f0ffe).rw(FUNC(pgm_state::bg_scrollx_r), FUNC(pgm_state::bg_scrollx_w));
	map(0xb04000, 0xb04001).mirror(0x0f0ffe).rw(FUNC(pgm_state::videoreg_4000_r), FUNC(pgm_state::videoreg_4000_w));
	map(0xb05000, 0xb05001).mirror(0x0f0ffe).rw(FUNC(pgm_state::tx_scrolly_r), FUNC(pgm_state::tx_scrolly_w));
	map(0xb06000, 0xb06001).mirror(0x0f0ffe).rw(FUNC(pgm_state::tx_scrollx_r), FUNC(pgm_state::tx_scrollx_w));
	map(0xb07000, 0xb07001).mirror(0x0f0ffe).r(FUNC(pgm_state::scanline_r)); // scanline counter?
	//map(0xb0c000, 0xb0c001).mirror(0x0f0ffe).nopr(); // access here cause video refresh?
	//map(0xb0d000, 0xb0d001).mirror(0x0f0ffe).nopr(); // access here cause video refresh?
	map(0xb0e000, 0xb0e001).mirror(0x0f0ffe).rw(FUNC(pgm_state::videoreg_e000_r), FUNC(pgm_state::videoreg_e000_w));

	map(0xc00003, 0xc00003).mirror(0x0e7ff0).r(m_soundlatch[0], FUNC(generic_latch_8_device::read)).w(FUNC(pgm_state::m68k_l1_w));
	map(0xc00005, 0xc00005).mirror(0x0e7ff0).rw(m_soundlatch[1], FUNC(generic_latch_8_device::read), FUNC(generic_latch_8_device::write));
	...
	map(0xc0000d, 0xc0000d).mirror(0x0e7ff0).rw(m_soundlatch[2], FUNC(generic_latch_8_device::read), FUNC(generic_latch_8_device::write));
...
{
	pgm_base_mem(map);
	map(0x000000, 0x0fffff).rom().mirror(0x700000).region("bios", 0);   // BIOS ROM
}

void pgm_state::pgm_basic_mem(address_map &map)
{
	pgm_mem(map);
	map(0x100000, 0x7fffff).rom().region("cart", 0); // Game ROM
}
...
{
	save_item(NAME(m_z80_bus_connect));
...
void pgm_state::pgmbase(machine_config &config)
{
	...
	m_maincpu->set_addrmap(AS_PROGRAM, &pgm_state::pgm_mem);
	TIMER(config, "scantimer").configure_scanline(FUNC(pgm_state::interrupt), m_screen, 0, 1);
	...
	m_screen->set_raw(50_MHz_XTAL/5, 640, 0, 448, 264, 0, 224); // or 20MHz / 2? framerate verified
	...
	m_ics->irq().set_inputline(m_soundcpu, 0);
...
#define ROM_LOAD16_WORD_SWAP_BIOS_x8(bios,name,offset,length,hash) \
		ROMX_LOAD(name, offset + 0 * length, length, hash, ROM_GROUPWORD | ROM_REVERSE | ROM_BIOS(bios)) \
		ROM_RELOAD(     offset + 1 * length, length) \
		ROM_RELOAD(     offset + 2 * length, length) \
		ROM_RELOAD(     offset + 3 * length, length) \
		ROM_RELOAD(     offset + 4 * length, length) \
		ROM_RELOAD(     offset + 5 * length, length) \
		ROM_RELOAD(     offset + 6 * length, length) \
		ROM_RELOAD(     offset + 7 * length, length)

#define ROM_LOAD16_WORD_SWAP_x2(name, offset, length, hash) \
		ROM_LOAD16_WORD_SWAP(name, offset + 0 * length, length, hash) \
		ROM_RELOAD(                offset + 1 * length, length)

#define ROM_LOAD16_WORD_SWAP_x4(name, offset, length, hash) \
		ROM_LOAD16_WORD_SWAP(name, offset + 0 * length, length, hash) \
		ROM_RELOAD(                offset + 1 * length, length) \
		ROM_RELOAD(                offset + 2 * length, length) \
		ROM_RELOAD(                offset + 3 * length, length)

#define ROM_LOAD16_WORD_SWAP_x8(name, offset, length, hash) \
		ROM_LOAD16_WORD_SWAP(name, offset + 0 * length, length, hash) \
		ROM_RELOAD(                offset + 1 * length, length) \
		ROM_RELOAD(                offset + 2 * length, length) \
		ROM_RELOAD(                offset + 3 * length, length) \
...
#define PGM_68K_BIOS \
	ROM_REGION16_BE( 0x100000, "bios", 0 ) /* 68000 Bios */ \
	ROM_SYSTEM_BIOS( 0, "v2",     "PGM Bios V2" ) \
	ROM_LOAD16_WORD_SWAP_BIOS_x8( 0, "pgm_p02s.u20",    0x00000, 0x020000, CRC(78c15fa2) SHA1(885a6558e022602cc6f482ac9667ba9f61e75092) ) /* Version 2 (Label: IGS | PGM P02S | 1P0792D1 | J992438 )*/ \
	ROM_SYSTEM_BIOS( 1, "v1",     "PGM Bios V1" ) \
	ROM_LOAD16_WORD_SWAP_BIOS_x8( 1, "pgm_p01s.u20",    0x00000, 0x020000, CRC(e42b166e) SHA1(2a9df9ec746b14b74fae48b1a438da14973702ea) ) /* Version 1 */
...
ROM_START( pgm )
	...
	ROM_REGION( 0x200000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( orlegend )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0103.rom",    0x000000, 0x200000, CRC(d5e93543) SHA1(f081edc26514ca8354c13c7f6f89aba8e4d3e7d2) )
	...
	ROM_REGION( 0x600000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( orlegende )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0102.u2",    0x000000, 0x200000, CRC(4d0f6cc5) SHA1(8d41f0a712fb11a1da865f5159e5e27447b4388a) )
...
ROM_START( orlegendc )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0101.u2",    0x000000, 0x200000, CRC(b24f0c1e) SHA1(a2cf75d739681f091c24ef78ed6fc13aa8cfe0c6) )
...
ROM_START( orlegendca )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0101.102",    0x000000, 0x200000, CRC(7a22e1cb) SHA1(4fe0fde00521b0915146334ea7213f3eb7e2affc) )
...
ROM_START( orlegend111c )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "olv_111_china_u6.u6",     0x000001, 0x080000, CRC(5fb86373) SHA1(2fc58eff1f38754c75819fde666244b867ca4f05) )
	ROM_LOAD16_BYTE( "olv_111_china_u9.u9",     0x000000, 0x080000, CRC(83cf09c8) SHA1(959780b45326059517f3008a356657f4f3d2908f) )
	ROM_LOAD16_BYTE( "olv_111_china_u7.u7",     0x100001, 0x080000, CRC(6ee79faf) SHA1(039b4b07b8577f0d3022ae01210c00375624cb3c) )
	ROM_LOAD16_BYTE( "olv_111_china_u11.u11",   0x100000, 0x080000, CRC(b80ddd3c) SHA1(55c700ce71ffdee392e03fd9d4719542c3527132) )
...
ROM_START( orlegend111t )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "olv_111_taiwan_u6.u6",     0x000001, 0x080000, CRC(b205a733) SHA1(33f4c9162e36be4957004f80593f94fc33b163f8) )
	ROM_LOAD16_BYTE( "olv_111_taiwan_u9.u9",     0x000000, 0x080000, CRC(6d9d29b4) SHA1(29a18de7e5b58c2f3125d6dc9cc8a8186180e956) )
	ROM_LOAD16_BYTE( "olv_111_taiwan_u7.u7",     0x100001, 0x080000, CRC(27628e87) SHA1(a0effd83dc57ac72ba4f110737a075705d78e798) )
	ROM_LOAD16_BYTE( "olv_111_taiwan_u11.u11",   0x100000, 0x080000, CRC(23f33bc9) SHA1(f24490370d40d905afe8b716a3953b4e9f0aada4) )
...
ROM_START( orlegend111k )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "olv_111_korea_u6.u6",     0x000001, 0x080000, CRC(1ff35baa) SHA1(f6791cc37ea468d0154b0e31a99f6f33a74bca81) )
	ROM_LOAD16_BYTE( "olv_111_korea_u9.u9",     0x000000, 0x080000, CRC(87b6d202) SHA1(32828166a645158630f79e7d493a2774e69bc265) )
	ROM_LOAD16_BYTE( "olv_111_korea_u7.u7",     0x100001, 0x080000, CRC(27628e87) SHA1(a0effd83dc57ac72ba4f110737a075705d78e798) )
	ROM_LOAD16_BYTE( "olv_111_korea_u11.u11",   0x100000, 0x080000, CRC(23f33bc9) SHA1(f24490370d40d905afe8b716a3953b4e9f0aada4) )
...
ROM_START( orlegend105t )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "olv_105_taiwan_u6.u6",     0x000001, 0x080000, CRC(2b14331f) SHA1(3909b5480764916761895ad3f25dd4f40762a2d8) )
	ROM_LOAD16_BYTE( "olv_105_taiwan_u9.u9",     0x000000, 0x080000, CRC(ae9d71e4) SHA1(20f84c18808a80464f22c72eafe7680690274f39) )
	ROM_LOAD16_BYTE( "olv_105_taiwan_u7.u7",     0x100001, 0x080000, CRC(5712facc) SHA1(2d95ebd1703874e89ac3a206f8c1f0ece6e833e0) )
	ROM_LOAD16_BYTE( "olv_105_taiwan_u11.u11",   0x100000, 0x080000, CRC(40ae4d9e) SHA1(62d7a96438b7fe93f74753333f50e077d417971e) )
...
ROM_START( orlegend105k )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "olv_105_korea_u6.u6",     0x000001, 0x080000, CRC(b86703fe) SHA1(a3529b45efd400ecd5e76f764b528ebce46e24ab) )
	ROM_LOAD16_BYTE( "olv_105_korea_u9.u9",     0x000000, 0x080000, CRC(5a108e39) SHA1(2033f4fe3f2dfd725dac535324f58348b9ac3914) )
	ROM_LOAD16_BYTE( "olv_105_korea_u7.u7",     0x100001, 0x080000, CRC(5712facc) SHA1(2d95ebd1703874e89ac3a206f8c1f0ece6e833e0) )
	ROM_LOAD16_BYTE( "olv_105_korea_u11.u11",   0x100000, 0x080000, CRC(40ae4d9e) SHA1(62d7a96438b7fe93f74753333f50e077d417971e) )
...
ROM_START( drgw2 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x2( "dragon_ii_v-110x.u2",    0x000000, 0x080000, CRC(1978106b) SHA1(af8a13d7783b755a58762c98bdc32cab845b2251) )
	...
	ROM_REGION( 0x400000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
ROM_END
...
ROM_START( drgw2100x )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x2( "dragon_ii_v-100x.u2",    0x000000, 0x080000,  CRC(5e71851d) SHA1(62052469f69daec88efd26652c1b893d6f981912) )
...
ROM_START( drgw2101c )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x2( "dragon_ii_v-101c.u2",    0x000000, 0x080000, CRC(b0c592fa) SHA1(87ccfdb940303ebcf42cb2952aecae97648c1e0d) )
...
ROM_START( drgw2100c )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x2( "dragon_ii_v-100c.u2",    0x000000, 0x080000, CRC(67467981) SHA1(58af01a3871b6179fe42ff471cc39a2161940043) )
...
ROM_START( drgw2100j )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x2( "dragon_ii_v100j.u2",    0x000000, 0x080000, CRC(f8f8393e) SHA1(ef0db668b4e4f661d4c1e95d57afe881bcdf13cc) )
...
ROM_START( drgw2100hk ) // the IGS025 has a "DRAGON-II 0004-1" sticker, the IGS012 has no per-game marking
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x2( "dragon_ii_v-100-h.u2",    0x000000, 0x080000, CRC(c6e2e6ec) SHA1(84145dfb26857ea20efb233363f175bc9bb25b0c) )
...
ROM_START( drgw3 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "dw3_v106_u12.u12",     0x000001, 0x080000,  CRC(c3f6838b) SHA1(c135b1d4dd62af308139d40d03c29be7508fb1e7) )
	ROM_LOAD16_BYTE( "dw3_v106_u13.u13",     0x000000, 0x080000,  CRC(28284e22) SHA1(4643a69881ddb7383ca10f3eb2aa2cf41be39e9f) )
	...
	ROM_REGION( 0x800000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( drgw3105 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "dw3_v105_u12.u12",     0x000001, 0x080000,  CRC(c5e24318) SHA1(c6954495bbc72c3985df75aecf6afd6826c8e30e) )
	ROM_LOAD16_BYTE( "dw3_v105_u13.u13",     0x000000, 0x080000,  CRC(8d6c9d39) SHA1(cb79303ab551e91f07e11414db4254d5b161d415) )
...
ROM_START( drgw3103 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "dragon_iii_v103j_u12.u12",     0x000001, 0x080000,  CRC(275b39a2) SHA1(8ba4d2601734c2dda3d4269fbe8f543dc3f0b212) )
	ROM_LOAD16_BYTE( "dragon_iii_v103j_u13.u13",     0x000000, 0x080000,  CRC(9aa56e8f) SHA1(c3f27d8b59adf72040a2e2c11e34f9b07efd7e9e) )
...
ROM_START( drgw3100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "dw3_v100_u12.u12",     0x000001, 0x080000,  CRC(47243906) SHA1(9cd46e3cba97f049bcb238ceb6edf27a760ef831) )
	ROM_LOAD16_BYTE( "dw3_v100_u13.u13",     0x000000, 0x080000,  CRC(b7cded21) SHA1(c1ae2af2e42227503c81bbcd2bd6862aa416bd78) )
...
ROM_START( dwex )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "dwex_v100.u12",     0x000001, 0x080000, CRC(bc171799) SHA1(142329dffbca199f3e748a52146a03e27b36db6a) ) // V100 08/16/00 09:39:27
	ROM_LOAD16_BYTE( "dwex_v100.u13",     0x000000, 0x080000, CRC(7afe6322) SHA1(a52d71af1d6de16c5a3df23eacdab3466693ba8d) )
...
ROM_START( dwex101cn )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "dwex_v101cn.u12",     0x000001, 0x080000, CRC(4f951f42) SHA1(830a943ce34c63ce418f60d913fae333377a9704) ) // V101CN (China) / V100 (other regions) 12/12/01 09:45:00
	ROM_LOAD16_BYTE( "dwex_v101cn.u13",     0x000000, 0x080000, CRC(66172511) SHA1(eb1a6fc9c22f04fcca0395a4b5c2972438c60a78) )
...
ROM_START( kov )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0601_v117.u1",    0x000000, 0x400000, CRC(c4d19fe6) SHA1(14ef31539bfbc665e76c9703ee01b12228344052) )
...
ROM_START( kov115 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0600.115",    0x000000, 0x400000, CRC(527a2924) SHA1(7e3b166dddc5245d7b408e78437c16fd2986d1d9) )
...
ROM_START( kov114 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "sav114_u4.u4",        0x000001, 0x080000, CRC(4db3d4d3) SHA1(924734ec3b3d6de21872890b9575f72f81c7b636) )
	ROM_LOAD16_BYTE( "sav114_u7.u7",        0x000000, 0x080000, CRC(b8d12b0c) SHA1(f4c02e494a479c3021b56bb87341d507104f93d5) )
	ROM_LOAD16_BYTE( "sav114_u5.u5",        0x100001, 0x080000, CRC(9e586dab) SHA1(3de948decf2b7bcbbbc37d2e6fa7a2a71f0b8d5e) )
	ROM_LOAD16_BYTE( "sav114_u8.u8",        0x100000, 0x080000, CRC(ab129997) SHA1(b0e56a09df0def3e8b584ca6f53cd3c88634653e) )
	ROM_LOAD16_WORD_SWAP( "sav114_u10.u10", 0x200000, 0x080000, CRC(8f84ecfd) SHA1(2e7f322da6c4b1d6daf7a308229f4cf2e69fda8f) )
...
ROM_START( kov111 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "sav111_u4.u4",        0x000001, 0x080000, CRC(ae2f1b4e) SHA1(2ac9d84f5dee52f374941cfd68e2b98ecad436a8) )
	ROM_LOAD16_BYTE( "sav111_u7.u7",        0x000000, 0x080000, CRC(95eedf0e) SHA1(582a54e9a1eda7ff73e20f0e69d2d50141772378) )
	ROM_LOAD16_BYTE( "sav111_u5.u5",        0x100001, 0x080000, CRC(5fdd4aa8) SHA1(43c96e21ad4f11148e1e94a59c53780b2edd43ba) )
	ROM_LOAD16_BYTE( "sav111_u8.u8",        0x100000, 0x080000, CRC(003cbf49) SHA1(fb5bea47ecae025b1b425af52cd05e061f45e377) )
	ROM_LOAD16_WORD_SWAP( "sav111_u10.u10", 0x200000, 0x080000, CRC(d5536107) SHA1(f963e015d99c1621323eecf63e773c0b9f4b6a43) )
...
ROM_START( kovplus )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0603_v119.u1",    0x000000, 0x400000, CRC(e4b0875d) SHA1(e8382e131b0e431406dc2a05cc1ef128302d987c) )
...
ROM_START( kovplusa )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "v119.u4",         0x000001, 0x080000, CRC(6750388f) SHA1(869f4ad27f2992cc62baa9a78bf7984a43ec4cc5) )
	ROM_LOAD16_BYTE( "v119.u7",         0x000000, 0x080000, CRC(d4101ffd) SHA1(a327fd56eec65b07df9305cd93ef2c46bf8e40f3) )
	ROM_LOAD16_BYTE( "v119.u5",         0x100001, 0x080000, CRC(8200ece6) SHA1(97081d2e8aed2ac6fbe5951890aecea18af5ce2e) )
	ROM_LOAD16_BYTE( "v119.u8",         0x100000, 0x080000, CRC(71e28f27) SHA1(db382807e9185f0dc17124f210165fa1b36ca6ac) )
	ROM_LOAD16_WORD_SWAP( "v119.u10",   0x200000, 0x080000, CRC(29588ef2) SHA1(17d1a308d44434cf65224a24360cf4b6e32d28f3) )
...
ROM_START( kovsgqyz )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_sgqyz.rom",    0x000000, 0x400000, CRC(18e1eed9) SHA1(db18d9121bb533140957e9c58dbc38211d164b01) )
...
ROM_START( kovsgqyza )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_sgqyza.rom",    0x000000, 0x400000, CRC(5a30dcb7) SHA1(64f34faf99a19c0a54899990695129c512d5a3c8) )
...
ROM_START( kovsgqyzb )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_sgqyzb.rom",    0x000000, 0x400000, CRC(18b8b9c0) SHA1(f4937aa21cd11af16fb50e7a75c8d4c4ed27c5cf) )
...
ROM_START( kovsh )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0605_v104.u1",    0x000000, 0x400000, CRC(7c78e5f3) SHA1(9b1e4bd63fb1294ebeb539966842273c8dc7683b) )
	//ROM_LOAD16_WORD_SWAP( "kovsh-v0104-u1.bin",    0x000000, 0x400000, CRC(4e2ba39b) SHA1(f3b5aa6f45cfd5a7f1e2a2e893d1652a3f23d6b8) ) // identical but the last 1MB is filled with 0xff instead of 0x00
...
ROM_START( kovsh103 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0604_v103.u1",    0x000000, 0x400000, CRC(f0b3da82) SHA1(4067beb69c049b51bce6154f4cf880600ca4de11) )
...
ROM_START( kovsh102 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0600.102",    0x000000, 0x400000, CRC(fdd4fb0f) SHA1(6906ce68f37b82e52ba30e5cceb1304d9e01430a) )
...
ROM_START( kovsh101 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0600.101",    0x000000, 0x400000, CRC(517a9bcf) SHA1(1ee0333aee2a7569e15bb2a1be8dd03f8b08e08c) )
...
ROM_START( kovsh100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "kovsh-v0100-u1.bin",    0x000000, 0x400000, CRC(d145c1ca) SHA1(ce4da36791bc5eea9fe1ef6db180d789bab0bab7) )
...
ROM_START( kovqhsgs )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "qhsg_c51.rom", 0x000000, 0x400000, CRC(e5cbac85) SHA1(4b424206387057863990b04f6d5bd0b6f754814f) ) // V300CN V303CN Nov 21 2008 19:03:49
	...
	ROM_REGION( 0xc00000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( photoy2k )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "pgm_p0701_v105.u2",     0x000000, 0x200000, CRC(fab142e0) SHA1(8dc7e53b740ed68bac98c0ef7ca4943c517e6f5d) ) // 10/12/99 21:07:53
	...
	ROM_REGION( 0x480000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( photoy2k104 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v104.16m",     0x000000, 0x200000, CRC(e051070f) SHA1(a5a1a8dd7542a30632501af8d02fda07475fd9aa) ) // 06/10/99 17:28:34
...
ROM_START( photoy2k103j )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "photo_y2k_v103_u4.u4",  0x000001, 0x080000, CRC(c16dc699) SHA1(062d38ed32f56c1e640c5e2be046bc6e150123b1) ) // 05/10/99 13:32:52
	ROM_LOAD16_BYTE( "photo_y2k_v103_u6.u6",  0x000000, 0x080000, CRC(2e2671a4) SHA1(5f37b6d789fedeb5e291081f2908061f30875dc6) )
	ROM_LOAD16_BYTE( "photo_y2k_v103_u5.u5",  0x100001, 0x080000, CRC(97839a61) SHA1(bf34e3fab90a846baa5b5e0a3c3d9d99a603c8ee) )
	ROM_LOAD16_BYTE( "photo_y2k_v103_u8.u8",  0x100000, 0x080000, CRC(43af9664) SHA1(00dc74960cb126adfc223783b09a2787fe37625e) )
...
ROM_START( photoy2k102j )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "photo_y2k_v102_u4.u4",     0x000001, 0x080000, CRC(a65eda9f) SHA1(6307cacf4a262e781753eff14700a0455837780c) ) // 05/03/99 16:06:33
	ROM_LOAD16_BYTE( "photo_y2k_v102_u6.u6",     0x000000, 0x080000, CRC(b9ca5504) SHA1(058cf01316f233236ca9861349f515935283b75e) )
	ROM_LOAD16_BYTE( "photo_y2k_v102_u5.u5",     0x100001, 0x080000, CRC(9201621b) SHA1(1ca3ebe7eec40614bfa8b911657fa2b51f2c51a4) )
	ROM_LOAD16_BYTE( "photo_y2k_v102_u8.u8",     0x100000, 0x080000, CRC(3be22b8f) SHA1(03634fbd6a8a8369c6cb1fd6694a3784dac5bf59) )
...
ROM_START( py2k2 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "y2k2_m-101xx.u1",     0x000000, 0x200000, CRC(c47795f1) SHA1(5be4af4275571932d7740c3ea0857a1f58a3f6d9) ) // 68k (encrypted) 2nd half empty...; 05/25/01 11:02:54
	...
	ROM_REGION( 0x880000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( py2k2100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "y2kii_v100.u1",     0x000000, 0x100000, CRC(7a1e36ac) SHA1(0ff9a30733ca8026e4acce45a8993c8ab4b242b5) ) // 68k (encrypted) 08/07/00 13:19:59
...
ROM_START( pgm3in1 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v-102cn.u3",     0x000000, 0x200000, CRC(72e06b2b) SHA1(0d72f90b1a2df5f0e8708b59d2a7c13dba998acd)) // M68K V102 08/23/04 13:03:26 (encrypted)
	...
	ROM_REGION( 0xe80000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( pgm3in1c100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v-100cn.u3",     0x000000, 0x200000, CRC(a39f59b4) SHA1(4eb53fb9f173cb470e16dc8f193c8909cf045e3d)) // M68K V100 07/13/04 12:09:20 (encrypted)
...
ROM_START( killbld )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0300_v109.u9", 0x000000, 0x200000, CRC(2fcee215) SHA1(855281a9090bfdf3da9f4d50c121765131a13400) )
...
ROM_START( killbld104 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "kb_u3_v104.u3",     0x000001, 0x080000, CRC(6db1d719) SHA1(804002f014d275aaf0368fb7f904938fe4ac07ee) )
	ROM_LOAD16_BYTE( "kb_u6_v104.u6",     0x000000, 0x080000, CRC(31ecc978) SHA1(82666d534e4151775063af6d39f575faba0f1047) )
	ROM_LOAD16_BYTE( "kb_u4_v104.u4",     0x100001, 0x080000, CRC(1ed8b2e7) SHA1(331c037640cfc1fe743cd0e65a1156c470b3303e) )
	ROM_LOAD16_BYTE( "kb_u5_v104.u5",     0x100000, 0x080000, CRC(a0bafc29) SHA1(b20db7c16353c6f87ed3c08c9d037b07336711f1) )
...
ROM_START( killbld106 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "kb_u3_v106.u3",     0x000001, 0x080000, CRC(33b9111a) SHA1(26875a9e502af9a36d13077cb7b07f9d28773d72) )
	ROM_LOAD16_BYTE( "kb_u6_v106.u6",     0x000000, 0x080000, CRC(1c957bd7) SHA1(2bb54915166b4a62148de043b2c2088d39b91f14) )
	ROM_LOAD16_BYTE( "kb_u4_v106.u4",     0x100001, 0x080000, CRC(169bbaaf) SHA1(a1833d3fd024c43ba7642f13e83a5b7b66631136) )
	ROM_LOAD16_BYTE( "kb_u5_v106.u5",     0x100000, 0x080000, CRC(df85abd4) SHA1(f9e37f76c7af8a8492bd1fd22d8b3fbda194ed03) )
...
ROM_START( killbld100 ) // was dumped from a Taiwanese board
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "kb_u3_v100.u3",     0x000001, 0x080000, CRC(ba3233a9) SHA1(d41f00c0c83ead8567b2e4ebf3fd7b8525f06b73) )
	ROM_LOAD16_BYTE( "kb_u6_v100.u6",     0x000000, 0x080000, CRC(55ae1d39) SHA1(931411c0d7d02a636010bb5f7f312906de32d839) )
	ROM_LOAD16_BYTE( "kb_u4_v100.u4",     0x100001, 0x080000, CRC(d56a8407) SHA1(1c43c62ec9e3b11f77d9c14647797493f7b8f960) )
	ROM_LOAD16_BYTE( "kb_u5_v100.u5",     0x100000, 0x080000, CRC(99afff2b) SHA1(4167efc81daf365e632177bb26650c95dcc32ccb) )
...
ROM_START( puzlstar ) // V100MG 09/30/99 11:39:23
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "puzzlestar_u1_v100mg.u1",     0x000000, 0x080000, CRC(c6ee43d7) SHA1(2419414ded556b5e4868d51b0da5dfce374d6bc0) )
	ROM_LOAD16_BYTE( "puzzlestar_u2_v100mg.u2",     0x000001, 0x080000, CRC(42aa03ce) SHA1(8e1666cc3ded98179a3e4854973604172921bbd6) )
...
ROM_START( puzlstara ) // V100MG 09/20/99 15:16:02
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "v100mg.u1",     0x000000, 0x080000, CRC(5788b77d) SHA1(7770aae6e686da92b2623c977d1bc8f019f48267) )
	ROM_LOAD16_BYTE( "v100mg.u2",     0x000001, 0x080000, CRC(4c79d979) SHA1(3b92052a35994f2b3dd164930154184c45d5e2d0) )
...
ROM_START( olds103t )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0500.v103",0x000000, 0x400000, CRC(17e32e14) SHA1(b8f731087af2c59fe5b1da31f1cb055d35c8b440) )
	...
	ROM_REGION( 0x1000000, "ics", 0 ) // Samples
	PGM_AUDIO_BIOS
...
ROM_START( olds )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "sp_v101_u2.u2",      0x000001, 0x080000,   CRC(08eb9661) SHA1(105946e72e562adb1a9fd794ca0fd2c91967eb56) )
	ROM_LOAD16_BYTE( "sp_v101_u3.u3",      0x000000, 0x080000,   CRC(0a358c1e) SHA1(95c7c3f069c5d05001e22535750f6b3cd7de105f) )
	ROM_LOAD16_BYTE( "sp_v101_u4.u4",      0x100001, 0x080000,   CRC(766570e0) SHA1(e7c3f5664ec69b662b82c2e1375555db7305390c) )
	ROM_LOAD16_BYTE( "sp_v101_u5.u5",      0x100000, 0x080000,   CRC(58662e12) SHA1(2b39bd847e9c4968a8e77a2f3cec77cf323ceee3) )
	ROM_LOAD16_WORD_SWAP( "sp_v101_u1.u1", 0x200000, 0x080000,   CRC(2b2f4f1e) SHA1(67b97cf8cc7f517d67cd45588addd2ad8e24612a) )
...
ROM_START( olds100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "sp_v100_u2.u2",      0x000001, 0x080000,  CRC(517c2a06) SHA1(bbf5b311fac9b0bb4d4129c0561e5e24f6963fa2) )
	ROM_LOAD16_BYTE( "sp_v100_u3.u3",      0x000000, 0x080000,  CRC(d0e2b741) SHA1(2e671dbb4320d1f0c059b35efd33cdea26f12131) )
	ROM_LOAD16_BYTE( "sp_v100_u4.u4",      0x100001, 0x080000,  CRC(32a6bdbd) SHA1(a93d7f4eae722a58eca9ec351ad5890cefda56f0) )
	ROM_LOAD16_BYTE( "sp_v100_u5.u5",      0x100000, 0x080000,  CRC(b4a1cafb) SHA1(b2fccd480ede93f58ad043387b18b898152f06ef) )
	ROM_LOAD16_WORD_SWAP( "sp_v100_u1.u1", 0x200000, 0x080000,  CRC(37ea4e75) SHA1(a94fcb89da3394a43d360f885419677f511d2580) )
...
ROM_START( olds100a )
	...
	ROM_LOAD16_WORD_SWAP( "pgm_p0500_v100.u24",    0x000000, 0x400000, CRC(8981fc87) SHA1(678d6705d06b99bca5951ff77708adadc4c4396b) )
...
ROM_START( kov2 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v107_u18.u18",      0x000000, 0x400000, CRC(661a5b2c) SHA1(125054fabc93d4f4cba869c3e6adf863650d30cf) ) // 05/10/01 14:24:08 V107
...
ROM_START( kov2106 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v106_u18.u18",    0x000000, 0x400000, CRC(40051ad9) SHA1(ba2ddf267fe688d5dfed575aeeccbab10135b37b) ) // 02/27/01 13:26:46 V106
...
ROM_START( kov2104 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v104_u18.u18",    0x000000, 0x400000, CRC(0fdb050b) SHA1(8ac704459e1a17d0956dd3529e81b6bcd8abf361) ) // 01/19/01 10:19:38 V104
...
ROM_START( kov2103 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v103_u18.u18",    0x000000, 0x400000, CRC(98c32f76) SHA1(ec7e35e8071bb7097e415493be4e40be0ca19fd6) ) // 12/28/00 15:09:31 V103
...
ROM_START( kov21022 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v102_u18.u18",    0x000000, 0x400000, CRC(a2489c37) SHA1(77ea7cdec211848296dafd45bee1d042133ea2a6) ) // 12/14/00 10:33:36 V102
...
ROM_START( kov2101 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v101_u18.u18",    0x000000, 0x400000, CRC(c9926f35) SHA1(a9c72d0c5d239164107894c7d3fffe4af29ed201) ) // 12/07/00 16:40:30 V101
...
ROM_START( kov2100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v100_u18.u18",    0x000000, 0x400000, CRC(86205879) SHA1(f73d5b70b41d39be1cac75e474b025de2cce0b01) ) // 11/29/00 11:03:08 V100
...
ROM_START( kov2p )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v205_32m.u8",    0x000000, 0x400000, CRC(3a2cc0de) SHA1(d7511478b34bfb03b2fb5b8268b60502d05b9414) ) // 04/25/02 17:48:27 M205XX
...
ROM_START( kov2p204 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v204_32m.u8",    0x000000, 0x400000, CRC(583e0650) SHA1(2e5656dd9c6cba9f84af9baa3f5f70cdccf9db47) ) // 08/28/01 09:11:49 M204XX
...
ROM_START( kov2p203 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v203_32m.u8",    0x000000, 0x400000, CRC(11416886) SHA1(00088165893ed0b5fb8bbac3def0edeb9ff0c4fd) ) // 08/13/01 16:57:32 M203XX
...
ROM_START( kov2p202 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v202_32m.u8",    0x000000, 0x400000, CRC(e9b5aa0c) SHA1(39a776c8501e8557d305cfa56c997f9adeb6bcd2) ) // 07/09/01 11:03:50 M202XX
...
ROM_START( kov2p200 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v200_32m.u8",    0x000000, 0x400000, CRC(9a09fd61) SHA1(690a0b323e1f275a08ba84febf93bf8edc2d0802) ) // 06/18/01  22:59:12 M200XX
...
#define DDP2_COMMON_ROMS \
	...
	ROM_REGION( 0x800000, "ics", ROMREGION_ERASE00 ) /* Samples */ \
	PGM_AUDIO_BIOS \
...
#define DDP2_PROGRAM_102 \
	ROM_REGION16_BE( 0x800000, "cart", 0 ) /* 68000 Code */ \
	ROM_LOAD16_WORD_SWAP( "v102.u8", 0x000000, 0x200000, CRC(5a9ea040) SHA1(51eaec46c368f7cfc5245e64896092f52b1193e0) ) // 09/10/01
#define DDP2_PROGRAM_101 \
	ROM_REGION16_BE( 0x800000, "cart", 0 ) /* 68000 Code */ \
	ROM_LOAD16_WORD_SWAP( "v101_16m.u8", 0x000000, 0x200000, CRC(5e5786fd) SHA1(c6fc2956b5dc6a97c0d7d808a8c58aa21fa023b9) ) // 07/10/01
#define DDP2_PROGRAM_100 \
	ROM_REGION16_BE( 0x800000, "cart", 0 ) /* 68000 Code */ \
	ROM_LOAD16_WORD_SWAP( "v100_u8.u8", 0x000000, 0x200000, CRC(0c8aa8ea) SHA1(57e33224622607a1df8daabf26ba063cf8a6d3fc) ) // 05/21/01
...
ROM_START( dw2001 )
	...
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x8( "dw2001_u22.u22", 0x000000, 0x80000, CRC(5cabed92) SHA1(d513e353c5c4695b16228e0bda9388c396aa4a81) ) // 02/21/01 16:05:16
...
ROM_START( dwpc )
	...
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "dwpc_v110cn_u22.u22", 0x000000, 0x80000, CRC(64f22362) SHA1(5f7c58498ae5cbec1c36eaa65c75287c36b6bd52) ) // 03/19/02 11:13:16
...
ROM_START( dwpc101j )
	...
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "dwpc_v101jp_u22.u22", 0x000000, 0x80000, CRC(b93027c0) SHA1(602e5f651ccb63e6465ebd7762d8d2dcf7d54077) ) // 09/26/01 10:23:26
...
ROM_START( puzzli2 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v100_u5.u5",     0x000000, 0x200000, CRC(1abb4595) SHA1(860bb49efc3cb55b6b9846f5ab787d6fd586432d) ) // PUZZLI-2 V0001
...
ROM_START( puzzli2s )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_BYTE( "2sp_v200.u3",     0x000000, 0x080000, CRC(2a5ba8a6) SHA1(4c87b849fd6f39152e3e2ef699b78ce24b3fb6d0) ) // PUZZLI-2 V200
	ROM_LOAD16_BYTE( "2sp_v200.u4",     0x000001, 0x080000, CRC(fa5c86c1) SHA1(11c219722b891b775c0f7f9bc8276cdd8f74d657) )
...
ROM_START( martmast )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v104_32m.u9",    0x000000, 0x400000, CRC(cfd9dff4) SHA1(328eaf6ac49a73265ee4e0f992b1b1312f49877b) )
...
ROM_START( martmast103c )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v103_32m.u9",    0x000000, 0x400000, CRC(df5ffbe9) SHA1(cd37900b3741707f81077ca07565997031409131) )
...
ROM_START( martmast102c )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "mm_v102_u9.u9",    0x000000, 0x400000, CRC(bb24b92a) SHA1(442cb9e3f51727be82f71c078c5c3e49dc1a23f0) )
...
ROM_START( martmasttw )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "mm_v102_u9.u9",    0x000000, 0x400000, CRC(7eb41ed4) SHA1(7df7789db8f9110fe747f41e193b4a28096018e7) )
...
ROM_START( dmnfrnt ) // same romset has also been found on a single PCB board (set dmnfrnta in FBNeo)
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v105_16m.u5",    0x000000, 0x200000, CRC(bda083bd) SHA1(58d6438737a2c43aa8bbcb7f34fb51375b781b1c) )
...
ROM_START( dmnfrnt103 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v103_16m.u5",    0x000000, 0x200000, CRC(2ddafb3d) SHA1(c7d22e007952459de6d23a42ce32aab67b493fc3) )
...
ROM_START( dmnfrnt102 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v102_16m.u5",    0x000000, 0x200000, CRC(3d4d481a) SHA1(95953b8f31343389405cc722b4177ff5adf67b62) )
...
ROM_START( dmnfrntpcb )
	ROM_REGION16_BE( 0x100000, "bios", 0 ) // 68000 Bios
	ROM_LOAD16_WORD_SWAP_x8( "pgm_p02s.u42",    0x00000, 0x020000, CRC(78c15fa2) SHA1(885a6558e022602cc6f482ac9667ba9f61e75092) ) /* IGS PGM P02S 1A3708A1A0 S002838  (uses standard PGM v2 bios) */

	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "demon_front_v107kr_u43.u43",    0x000000, 0x200000,  CRC(671d8a31) SHA1(a0c2af67d7c56b4b355883892a47640fc72408a1) )
...
ROM_START( theglad )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v101_u6.u6",      0x000000, 0x080000, CRC(f799e866) SHA1(dccc3c903357c40c3cf85ac0ae8fc12fb0f853a6) ) // V101 05/15/03 09:00:32
...
ROM_START( theglad100 ) // is this actually a pre-v100 proto?
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "u6.rom",       0x000000, 0x080000, CRC(14c85212) SHA1(8d2489708e176a2c460498a13173be01f645b79e) ) // 01/06/03 09:27:02
...
ROM_START( theglad101 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v100.u6",       0x000000, 0x080000, CRC(bcf3b172) SHA1(df7e2808c0341be0a59eefa852c857a3a919223e) ) // 02/25/03 17:42:51
...
ROM_START( thegladpcb )
	ROM_REGION16_BE( 0x100000, "bios", 0 ) // 68000 Bios
	ROM_LOAD16_WORD_SWAP_x8( "bios.u42",      0x000000, 0x020000, CRC(517cf7a2) SHA1(f5720b29e3be6ec22be03a768618cb2a1aa4ade7) ) // V0001-001J

	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v100_u43.u43",  0x000000, 0x080000, CRC(bcf3b172) SHA1(df7e2808c0341be0a59eefa852c857a3a919223e) ) // 02/25/03 17:42:51
...
ROM_START( oldsplus )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v-205cn.u10",   0x000000, 0x400000, CRC(923f7246) SHA1(818ade79e9724f5a2b0cc5a647ae5d4ee0374799) )
...
ROM_START( oldsplus203 ) // only program ROM provided for this set
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v-203cn.u10",   0x000000, 0x400000, CRC(c728cadc) SHA1(7e6acd2535b1700fdefc048bd521c305d3170108) )
...
ROM_START( kovshp )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "p0600h_101.rom",   0x000000, 0x400000, CRC(e1d89a19) SHA1(30e11c145652d03464b14d3cd09e4f35fff6120e) )
...
ROM_START( kovshp100 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "s__s_m-100cn.u10",   0x000000, 0x400000, CRC(e251e8e4) SHA1(af5b7c81632a39e1450d932951bed634c76b84e8) )
...
ROM_START( kovytzy )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ytzy_v201cn.rom",   0x000000, 0x400000, CRC(f3705ea0) SHA1(e31ad474d0c2364311d21a8ce37d49919c7b999c) )
...
ROM_START( kovshxas )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "assg_v202cn.rom",   0x000000, 0x400000, CRC(3b7b627f) SHA1(b331148501f9349fbd5882fb3f184f6304e58646) ) // V202CN Oct 6 2008 09:59:26
...
ROM_START( kovlsqh )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "lsqh_v200cn.rom", 0x000000, 0x400000, CRC(9935a27a) SHA1(3075935293172466c4bd997dcb67f864ae26493e) )
...
ROM_START( kovlsqh2 )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "lsqh2_prg.rom",0x000000, 0x400000, CRC(d71e3d50) SHA1(bda78648bc176b0ded74118a8e340ee661bb930d) )
...
ROM_START( kovlsjb )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "lsjb_prg.rom", 0x000000, 0x400000, CRC(adf06b37) SHA1(be3c0af64de374046d28492ac49c01da1ec78e40) )
...
ROM_START( kovlsjba )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "lsjba_prg.rom",0x000000, 0x400000, CRC(8b42f456) SHA1(48796e48f6f1a5f68442cf15a6b195095d443a35) )
...
ROM_START( killbldp )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v300xx_u6.u6",     0x000000, 0x080000, CRC(b7fb8ec9) SHA1(e71b2d74269a82c7155b9818821156e128b68b28) ) // V300 05-09-16 11:58:20
...
ROM_START( svg )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "svg_v200_u30.u30",      0x000000, 0x080000, CRC(34c18f3f) SHA1(42d1edd0dcfaa5e44861c6a1d4cb24f51ba23de8) ) // V200 09/12/05 16:42:51
...
ROM_START( svgtw )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v101tw_u30.u30",      0x000000, 0x080000, CRC(8d0405e4) SHA1(b6175c9ffeaac531d28e7845cb34c673476e286a) ) // V100 03/14/05 20:04:08
...
ROM_START( svghk )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "v101hk_u30.u30",      0x000000, 0x080000, CRC(8d0405e4) SHA1(b6175c9ffeaac531d28e7845cb34c673476e286a) ) // V100 03/14/05 20:04:08
...
ROM_START( svgpcb )
	ROM_REGION16_BE( 0x100000, "bios", 0 ) // 68000 Bios
	ROM_LOAD16_WORD_SWAP_x8( "svg_bios.u49",        0x000000, 0x020000, CRC(3346401f) SHA1(28bd730b6026c1e521c95072d33c7bdcd19c1460) )

	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "svg_v100jp_u50.u50",  0x000000, 0x080000, CRC(8d0405e4) SHA1(b6175c9ffeaac531d28e7845cb34c673476e286a) ) // V100 03/14/05 20:04:08
...
ROM_START( happy6 )
	...
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x8( "s101xx_u5.u5",      0x000000, 0x080000, CRC(aa4646e3) SHA1(e6772cc480ddd3e1d199364b1f2ff93b973e6842) ) // V101 03/17/04 11:26:48
...
ROM_START( happy6101 )
	...
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x8( "v100cn_u5.u5",      0x000000, 0x080000, CRC(a25418e8) SHA1(acd7e7b69956cb4ce8e26c6420cb97bb4bf404e7) ) // V100 12/22/03 15:35:33
...
ROM_START( happy6100hk )
	...
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP_x8( "v100hk_u5.u5",      0x000000, 0x080000, CRC(a25418e8) SHA1(acd7e7b69956cb4ce8e26c6420cb97bb4bf404e7) ) // V100 12/22/03 15:35:33
...
ROM_START( ketbl ) // this assumes a Dodonpachi 2 Bee Storm cart was used
	...
	ROM_CONTINUE(0x300000,0x100000) // first half is empty, use 2nd half
	ROM_CONTINUE(0x000000,0x100000) // ""

	PGM_68K_BIOS // cartridge overlays BIOS ROM
	ROM_COPY( "cart", 0x300000, 0x000000, 0x100000 )
...
ROM_START( espgalbl ) // this assumes a Dodonpachi 2 Bee Storm cart was used
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	// the program rom in the cartridge actually ends up mapping OVER the motherboard bios rom, you don't get the PGM splash etc.
	ROM_LOAD16_WORD_SWAP( "espgaluda_u8.bin", 0x300000, 0x100000, CRC(6a92dd52) SHA1(d4b694c88deaeebb7b1c0ddbf29a06380b06426f) )
	ROM_CONTINUE(0x000000,0x300000) // for avoid overlay area
...
ROM_START( ddp3 )
	ROM_REGION16_BE( 0x100000, "bios", 0 ) // 68000 Bios
	ROM_LOAD16_WORD_SWAP_x2( "ddp3_bios.u37",    0x00000, 0x080000, CRC(b3cc5c8f) SHA1(02d9511cf71e4a0d6ca8fd9a1ef2c79b0d001824) ) // uses a standard PGM bios with the startup logos hacked out

	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddp3_v101_16m.u36",  0x000000, 0x200000,CRC(fba2180e) SHA1(59c1a76243e587c07215c8a76649401ef0bff7c7) ) // marked 怒首領蜂(dodonpachi) 3 V101 16M
...
ROM_START( ddpdoj )
	...
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddp3_v101.u36",  0x000000, 0x200000, CRC(195b5c1e) SHA1(f18d791c034b0a3d85888a92fb5d326ee3deb04f) ) // yes this one was actually marked v101 which goes against the standard Cave marking system
...
ROM_START( ddpdoja )
	...
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddp3_d_d_1_0.u36",  0x000000, 0x200000, CRC(5d3f85ba) SHA1(4c24ea206140863d456179750366921442e1d2b8) ) // marked v100
...
ROM_START( ddpdojb )
	...
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "dd_v100.u36",  0x000000, 0x200000, CRC(7da0c1e4) SHA1(aca2fe35ba0ab3628900fa2aba2d22fc4fd7046d) ) // marked v100
...
ROM_START( ddpdojp )
	ROM_REGION16_BE( 0x100000, "bios", 0 ) // 68000 Bios
	ROM_LOAD16_WORD_SWAP_x8( "pgmbios.u20.27c210",    0x00000, 0x020000, CRC(1d2a7c15) SHA1(025a9f2bb64887699bf7ccab0f2ccfc55c3ad75c) )

	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ca008.cod_prom.u13.27c322",  0x000000, 0x400000, CRC(2ba7fa3b) SHA1(c4c5425a2455cb95555d94bbf8afc83cf0b140e8) )
...
ROM_START( ddpdojblk )
	...
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddb10_10_8_434f.u45",  0x000000, 0x200000, CRC(d21561db) SHA1(66a0103bc5f17b28736b562e32807271a5afa261) )
...
ROM_START( ddpdojblka )
	...
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddb_1dot.u45",  0x000000, 0x200000, CRC(265f26cd) SHA1(91abc7fc4722f3d01d76a4c1ae14c4132e4e576c) )
...
ROM_START( ddpdojblkb )
	...
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddb10.u45",  0x000000, 0x200000, CRC(72b35510) SHA1(9a432e5e1ebe61aafd737b6acc905653e5af0d38) )
...
ROM_START( ddpdojblkbl )
	PGM_68K_BIOS
	ROM_REGION16_BE( 0x800000, "cart", 0 ) // 68000 Code
	ROM_LOAD16_WORD_SWAP( "ddp_doj_u1.bin",  0x000000, 0x400000, CRC(eb4ab06a) SHA1(187c37e5319395e36a1cf3626b53e08df615cc0c) )
...
GAME( 1997, pgm,          0,         pgmbase,                pgm,       pgm_state,       init_pgm,         ROT0,   "IGS", "PGM (Polygame Master) System BIOS", MACHINE_IS_BIOS_ROOT )
...
GAME( 2002, ddp3,         0,         pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi III (World, 2002.05.15 Master Ver)",                MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE )
GAME( 2002, ddpdoj,       ddp3,      pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi Dai-Ou-Jou (Japan, 2002.04.05.Master Ver, 68k Label V101)",  MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE ) // is there a v101 without the . after 05?
GAME( 2002, ddpdoja,      ddp3,      pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi Dai-Ou-Jou (Japan, 2002.04.05.Master Ver, 68k Label V100)",  MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE )
GAME( 2002, ddpdojb,      ddp3,      pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi Dai-Ou-Jou (Japan, 2002.04.05 Master Ver)",       MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE )
...
GAME( 2002, ddpdojblk,    ddp3,      pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi Dai-Ou-Jou Black Label (Japan, 2002.10.07.Black Ver, newer)",        MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE ) // Displays "2002.04.05.Master Ver" (old) or "2002.10.07.Black Ver" (new)
GAME( 2002, ddpdojblka,   ddp3,      pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi Dai-Ou-Jou Black Label (Japan, 2002.10.07.Black Ver, older)",        MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE ) // Displays "2002.04.05.Master Ver" (old) or "2002.10.07.Black Ver" (new)
GAME( 2002, ddpdojblkb,   ddp3,      pgm_arm_type1_sim,      ddp3,      pgm_arm_type1_state, init_ddp3,     ROT270, "Cave (AMI license)", "DoDonPachi Dai-Ou-Jou Black Label (Japan, 2002.10.07 Black Ver)",        MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE ) // Displays "2002.04.05.Master Ver" (old) or "2002.10.07 Black Ver" (new)
