...
/*========================================================================= */
/* This source implements the ADSR volume envelope of the SID4-chip. */
...
const u8 masterVolumeLevels[16] =
{
	0,  17,  34,  51,  68,  85, 102, 119,
	136, 153, 170, 187, 204, 221, 238, 255
};

static u16 masterAmplModTable[16*256];
...
static const float decayReleaseTimes[16] =
{
	...
	static u32 attackRates[16];
	static u32 decayReleaseRates[16];
#else
	static u32 attackRates[16];
	static u32 attackRatesP[16];
	static u32 decayReleaseRates[16];
	static u32 decayReleaseRatesP[16];
#endif

static const u32 attackTabLen = 255;
static u32 releaseTabLen;
static u32 releasePos[256];


void enveEmuInit( u32 updateFreq, int measuredValues )
{
	u32 i, j, k;
	...
	for ( i = 0; i < 16; i++ )
	{
		for ( j = 0; j < 256; j++ )
		{
			...
			if (measuredValues)
			{
				tmpVol = (u16) ((293.0*(1-exp(j/-130.0)))+4.0);
				if (j == 0)
					tmpVol = 0;
				if (tmpVol > 255)
					tmpVol = 255;
...
#elif defined(DIRECT_FIXPOINT)
		u32 scaledenvelen = (u32)floor(( attackTimes[i] * updateFreq ) / 1000UL );
		if (scaledenvelen == 0)
			scaledenvelen = 1;
		attackRates[i] = (attackTabLen << 16) / scaledenvelen;

		scaledenvelen = (u32)floor(( decayReleaseTimes[i] * updateFreq ) / 1000UL );
		if (scaledenvelen == 0)
			scaledenvelen = 1;
...
void enveEmuResetOperator(sid4Operator* pVoice)
{
	/* mute, end of R-phase */
...
static inline u16 enveEmuStartAttack(sid4Operator*);
static inline u16 enveEmuStartDecay(sid4Operator*);
static inline u16 enveEmuStartRelease(sid4Operator*);
static inline u16 enveEmuAlterAttack(sid4Operator*);
static inline u16 enveEmuAlterDecay(sid4Operator*);
static inline u16 enveEmuAlterSustain(sid4Operator*);
static inline u16 enveEmuAlterSustainDecay(sid4Operator*);
static inline u16 enveEmuAlterRelease(sid4Operator*);
static inline u16 enveEmuAttack(sid4Operator*);
static inline u16 enveEmuDecay(sid4Operator*);
static inline u16 enveEmuSustain(sid4Operator*);
static inline u16 enveEmuSustainDecay(sid4Operator*);
static inline u16 enveEmuRelease(sid4Operator*);
static inline u16 enveEmuMute(sid4Operator*);

static inline u16 enveEmuStartShortAttack(sid4Operator*);
static inline u16 enveEmuAlterShortAttack(sid4Operator*);
static inline u16 enveEmuShortAttack(sid4Operator*);
...
static inline void enveEmuEnveAdvance(sid4Operator* pVoice)
{
#ifdef SID_FPUENVE
...
static inline u16 enveEmuMute(sid4Operator* pVoice)
{
	return 0;
}
...
static inline u16 enveEmuRelease(sid4Operator* pVoice)
{
#ifdef SID_FPUENVE
	pVoice->enveStep = (u16)pVoice->fenveStep;
#endif
...
#endif
	{
		...
		return masterAmplModTable[ pVoice->masterVolumeAmplIndex + pVoice->enveVol ];
	}
	else
	{
#if defined(DIRECT_FIXPOINT) && !defined(SID_FPUENVE)
...
static inline u16 enveEmuAlterRelease(sid4Operator* pVoice)
{
	u8 release = pVoice->SIDSR & 0x0F;
#ifdef SID_FPUENVE
...
static inline u16 enveEmuStartRelease(sid4Operator* pVoice)
{
...
static inline u16 enveEmuSustain(sid4Operator* pVoice)
{
	return masterAmplModTable[pVoice->masterVolumeAmplIndex+pVoice->enveVol];
}

static inline u16 enveEmuSustainDecay(sid4Operator* pVoice)
{
#ifdef SID_FPUENVE
	pVoice->enveStep = (u16)pVoice->fenveStep;
#endif
...
static inline u16 enveEmuAlterSustainDecay(sid4Operator* pVoice)
{
	u8 decay = pVoice->SIDAD & 0x0F ;
#ifdef SID_FPUENVE
...
static inline u16 enveEmuAlterSustain(sid4Operator* pVoice)
{
	if ( pVoice->enveVol > pVoice->enveSusVol )
	{
...
static inline u16 enveEmuDecay(sid4Operator* pVoice)
{
#ifdef SID_FPUENVE
	pVoice->enveStep = (u16)pVoice->fenveStep;
#endif
...
static inline u16 enveEmuAlterDecay(sid4Operator* pVoice)
{
	u8 decay = pVoice->SIDAD & 0x0F ;
#ifdef SID_FPUENVE
	pVoice->fenveStepAdd = decayReleaseRates[decay];
...
static inline u16 enveEmuStartDecay(sid4Operator* pVoice)
{
...
static inline u16 enveEmuAttack(sid4Operator* pVoice)
{
#ifdef SID_FPUENVE
	pVoice->enveStep = (u16)pVoice->fenveStep;
#endif
...
static inline u16 enveEmuAlterAttack(sid4Operator* pVoice)
{
	u8 attack = pVoice->SIDAD >> 4;
#ifdef SID_FPUENVE
...
static inline u16 enveEmuStartAttack(sid4Operator* pVoice)
{
...
static inline u16 enveEmuShortAttack(sid4Operator* pVoice)
{
#ifdef SID_FPUENVE
	pVoice->enveStep = (u16)pVoice->fenveStep;
#endif
...
static inline u16 enveEmuAlterShortAttack(sid4Operator* pVoice)
{
	u8 attack = pVoice->SIDAD >> 4;
#ifdef SID_FPUENVE
	pVoice->fenveStepAdd = attackRates[attack];
...
static inline u16 enveEmuStartShortAttack(sid4Operator* pVoice)
{
