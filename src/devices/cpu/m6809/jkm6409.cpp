...
000809 TJL:
    Started converting m6809 into jkm6409
...
DEFINE_DEVICE_TYPE(JKM6409, jkm6409_device, "jkm6409", "MATRIX JKM6409")


//-------------------------------------------------
//  jkm6409_device - constructor
//-------------------------------------------------

jkm6409_device::jkm6409_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	m6809_base_device(mconfig, tag, owner, clock, JKM6409, 1),
	m_io_config("io", ENDIANNESS_BIG, 8, 16),
	m_md(0),
	m_temp_im(0)
{
	m_program_config.addr_width(28);
	m_sprogram_config.addr_width(28);
}
...
void jkm6409_device::device_start()
{
	m_mintf = std::make_unique<mi_jkm6409>();
	m_mintf->m_io  = &space(AS_IO);
	super::device_start();

	// register our state for the debugger
	state_add(JKM6409_MD,    "MD",    m_md).mask(0xff);
	state_add(JKM6409_V,     "V",     m_v.w).mask(0xffff);
	state_add(JKM6409_A,     "A",     m_q.r.a).mask(0xff);
	state_add(JKM6409_B,     "B",     m_q.r.b).mask(0xff);
	state_add(JKM6409_D,     "D",     m_q.r.d).mask(0xffff);
	state_add(JKM6409_E,     "E",     m_q.r.e).mask(0xff);
	state_add(JKM6409_F,     "F",     m_q.r.f).mask(0xff);
	state_add(JKM6409_W,     "W",     m_q.r.w).mask(0xffff);
	state_add(JKM6409_Q,     "Q",     m_q.q).mask(0xffffffff);
	state_add(JKM6409_X,     "X",     m_x.w).mask(0xffff);
	state_add(JKM6409_Y,     "Y",     m_y.w).mask(0xffff);
	state_add(JKM6409_U,     "U",     m_u.w).mask(0xffff);
	state_add(JKM6409_MMR0,  "MMR0",  m_mmr[0].w).mask(0xffff);
	state_add(JKM6409_MMR1,  "MMR1",  m_mmr[1].w).mask(0xffff);
	state_add(JKM6409_MMR2,  "MMR2",  m_mmr[2].w).mask(0xffff);
	state_add(JKM6409_MMR3,  "MMR3",  m_mmr[3].w).mask(0xffff);
	state_add(JKM6409_MMR4,  "MMR4",  m_mmr[4].w).mask(0xffff);
	state_add(JKM6409_MMR5,  "MMR5",  m_mmr[5].w).mask(0xffff);
	state_add(JKM6409_MMR6,  "MMR6",  m_mmr[6].w).mask(0xffff);
	state_add(JKM6409_MMR7,  "MMR7",  m_mmr[7].w).mask(0xffff);
	state_add(JKM6409_MMR8,  "MMR8",  m_mmr[8].w).mask(0xffff);
	state_add(JKM6409_MMR9,  "MMR9",  m_mmr[9].w).mask(0xffff);
	state_add(JKM6409_MMR10, "MMR10", m_mmr[10].w).mask(0xffff);
	state_add(JKM6409_MMR11, "MMR11", m_mmr[11].w).mask(0xffff);
	state_add(JKM6409_MMR12, "MMR12", m_mmr[12].w).mask(0xffff);
	state_add(JKM6409_MMR13, "MMR13", m_mmr[13].w).mask(0xffff);
	state_add(JKM6409_MMR14, "MMR14", m_mmr[14].w).mask(0xffff);
	state_add(JKM6409_MMR15, "MMR15", m_mmr[15].w).mask(0xffff);
...
void jkm6409_device::device_reset()
{
	super::device_reset();
...
void jkm6409_device::device_pre_save()
{
	if      (m_reg8 == &m_q.r.a)    m_reg = JKM6409_A;
	else if (m_reg8 == &m_q.r.b)    m_reg = JKM6409_B;
	else if (m_reg8 == &m_q.r.e)    m_reg = JKM6409_E;
	else if (m_reg8 == &m_q.r.f)    m_reg = JKM6409_F;
	else if (m_reg8 == &m_cc)       m_reg = JKM6409_CC;
	else if (m_reg8 == &m_dp)       m_reg = JKM6409_DP;
	else if (m_reg8 == &m_md)       m_reg = JKM6409_MD;
	else if (m_reg8 == &m_temp.b.l) m_reg = JKM6409_ZERO_BYTE;

	else if (m_reg16 == &m_q.p.d)   m_reg = JKM6409_D;
	else if (m_reg16 == &m_x)       m_reg = JKM6409_X;
	else if (m_reg16 == &m_y)       m_reg = JKM6409_Y;
	else if (m_reg16 == &m_u)       m_reg = JKM6409_U;
	else if (m_reg16 == &m_s)       m_reg = JKM6409_S;
	else if (m_reg16 == &m_pc)      m_reg = JKM6409_PC;
	else if (m_reg16 == &m_q.p.w)   m_reg = JKM6409_W;
	else if (m_reg16 == &m_v)       m_reg = JKM6409_V;
	else if (m_reg16 == &m_temp)    m_reg = JKM6409_ZERO_WORD;
	else
		m_reg = 0;
}
...
void jkm6409_device::device_post_load()
{
	m_reg8 = nullptr;
	m_reg16 = nullptr;
...
device_memory_interface::space_config_vector jkm6409_device::memory_space_config() const
{
...
std::unique_ptr<util::disasm_interface> jkm6409_device::create_disassembler()
{
	return std::make_unique<jkm6409_disassembler>();
}
...
inline uint8_t jkm6409_device::read_operand()
{
...
inline uint8_t jkm6409_device::read_operand(int ordinal)
{
	switch(m_addressing_mode)
	{
...
inline void jkm6409_device::write_operand(uint8_t data)
{
	switch(m_addressing_mode)
	{
...
inline void jkm6409_device::write_operand(int ordinal, uint8_t data)
{
	switch(m_addressing_mode)
	{
...
inline uint8_t &jkm6409_device::bittest_register()
{
...
inline bool jkm6409_device::bittest_source()
{
...
inline bool jkm6409_device::bittest_dest()
{
...
inline void jkm6409_device::bittest_set(bool result)
{
	if (result)
...
inline m6809_base_device::exgtfr_register jkm6409_device::read_exgtfr_register(uint8_t reg)
{
	uint16_t value;
...
inline void jkm6409_device::write_exgtfr_register(uint8_t reg, m6809_base_device::exgtfr_register value)
{
	switch(reg & 0x0F)
	{
...
inline bool jkm6409_device::tfr_read(uint8_t opcode, uint8_t arg, uint8_t &data)
{
	PAIR16 *reg;
...
inline bool jkm6409_device::tfr_write(uint8_t opcode, uint8_t arg, uint8_t data)
{
	PAIR16 *reg;
...
void jkm6409_device::register_register_op()
{
...
inline void jkm6409_device::execute_one()
{
	switch(pop_state())
	{
#include "cpu/m6809/jkm6409.hxx"
	}
}
...
void jkm6409_device::execute_run()
{
	do
	{
		execute_one();
...
uint8_t jkm6409_device::mi_jkm6409::read_port(uint16_t adr)
{
	return m_io->read_byte(adr);
}

void jkm6409_device::mi_jkm6409::write_port(uint16_t adr, uint8_t val)
{
	m_io->write_byte(adr, val);
}


bool jkm6409_device::memory_translate(int spacenum, int intention, offs_t &address)
{
...
void jkm6409_device::set_mmr()
{
	...
	switch((operand >> 4) & 0x0F)
	{
		case  0: m_mmr[operand & 0x0f] = m_q.r.d;                                        break;  // D
		case  1: m_mmr[operand & 0x0f] = m_x.w;                                          break;  // X
		case  2: m_mmr[operand & 0x0f] = m_y.w;                                          break;  // Y
		case  3: m_mmr[operand & 0x0f] = m_u.w;                                          break;  // U
		case  4: m_mmr[operand & 0x0f] = m_s.w;                                          break;  // S
		case  5: m_mmr[operand & 0x0f] = m_pc.w;                                         break;  // PC
		case  6: m_mmr[operand & 0x0f] = m_q.r.w;                                        break;  // W
		case  7: m_mmr[operand & 0x0f] = m_v.w;                                          break;  // V
	}
}

u16 jkm6409_device::get_mmr()
{
	uint8_t operand = read_opcode_arg();
