...
#ifndef MAME_VIDEO_315_53134_H
#define MAME_VIDEO_315_53134_H
...
class sega315_53134_device : public sega315_53134_mode4_device
{
public:
	template <typename T>
	sega315_53134_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock, T &&cpu_tag)
		: sega315_53134_device(mconfig, tag, owner, clock)
	{
		m_cpu68k.set_tag(std::forward<T>(cpu_tag));
	}

	sega315_53134_device(const machine_config &mconfig, const char *tag, device_t *owner, u32 clock);
...
private:
	...
	inline void draw_tile(nametable_t tile, int start, int end, int &dpos, u64 pixshift);
	...
	void insta_68k_to_cram_dma(u32 source, u32 length);
	void insta_68k_to_vsram_dma(u32 source, u32 length);
	void insta_68k_to_vram_dma(u32 source, u32 length);
	void insta_vram_copy(u32 source, u32 length);

	void vdp_vram_write(u16 data);
	void vdp_cram_write(u16 data);
	void write_cram_value(int offset, u32 data);
	...
	std::unique_ptr<u64[]> m_sprite_renderline;
	std::unique_ptr<u64[]> m_highpri_renderline;
	std::unique_ptr<u64[]> m_video_renderline;
...
DECLARE_DEVICE_TYPE(SEGA315_53134, sega315_53134_device)

#endif // MAME_VIDEO_315_53134_H
