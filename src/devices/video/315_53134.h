...
class sega315_53134_device : public sega315_5313_mode4_device
{
public:
	template <typename T>
	sega315_53134_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&cpu_tag)
		: sega315_53134_device(mconfig, tag, owner, clock)
	{
		m_cpu68k.set_tag(std::forward<T>(cpu_tag));
	}

	sega315_53134_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
private:
	...
	std::unique_ptr<uint32_t[]> m_sprite_renderline;
	std::unique_ptr<uint32_t[]> m_highpri_renderline;
